...
const toggleCart = (event) => {
  if (event.target.id.includes("fav-btn")) {
    ...
    const index = referenceList.findIndex(cartId => cartId.id === Number(id))
...
const buttonFilter = (event) => {
  ...
  if(event.target.id.includes('books')) {
    const books = referenceList.filter(item => item.type.toLowerCase() === "book"); // .toLowerCase will take a lowercase string and will return whether or not the string is upper or lower case. .toUpperCase will do the same, but will take uppercase letters in the tripple equal
    renderCards(books)
  }
